export class UserController {
    ...
    async handlePurchaseAdded({ user, product }) {
        ...
        this.#userView.addPastPurchase(lastWatched);
        this.#events.dispatchUsersUpdated({ users: await this.#userService.getUsers() });
    }

    async handlePurchaseRemove({ userId, product }) {
        const user = await this.#userService.getUserById(userId);
        const index = user.watched.findIndex(item => item.id === product.id);

        if (index !== -1) {
            user.watched.splice(index, 1); // directly remove one item at the found index
    ...
    async displayUserDetails(user) {
        ...
        this.#userView.renderPastPurchases(user.watched);
