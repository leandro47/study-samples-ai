...
export class ModelView extends View {
    ...
    renderAllUsersPurchases(users) {
        const html = users.map(user => {
            const watchedHtml = (user.watched || []).map(movie => {
                return `<span class="badge bg-light text-dark me-1 mb-1">${movie.name}</span>`;
            }).join('');
            ...
            return `
                <div class="user-purchase-summary">
                    ...
                    <div class="purchases-badges">
                        ${watchedHtml || '<span class="text-muted">No watched movies</span>'}
                    </div>
                </div>
            `;
        }).join('');
